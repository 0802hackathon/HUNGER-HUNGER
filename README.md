...
GitHubの情報設計を参考に、暗色ヘッダー、Repository一覧風カード、README中心の詳細、Topicラベル、タブ、境界線主体の階層を採用しています。ログイン画面では認証Providerの識別に各社ロゴを使用し、それ以外の既存作品のロゴ・画像・キャラクター・台詞・固有フォントは複製していません。
